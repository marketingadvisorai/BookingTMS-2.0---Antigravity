...
export function useGames(venueId?: string) {
  ...
  const createGame = async (gameData: Omit<Game, 'id' | 'created_at' | 'updated_at' | 'created_by'>) => {
    ...
    try {
      ...
      // Step 4: Validate required fields before insert
      if (!cleanedGameData.venue_id) {
        throw new Error('venue_id is required but missing');
      }
      if (!cleanedGameData.name) {
        throw new Error('Game name is required but missing');
      }
      if (!cleanedGameData.price || cleanedGameData.price <= 0) {
        throw new Error('Valid price is required');
      }
      if (!cleanedGameData.duration || cleanedGameData.duration <= 0) {
        throw new Error('Valid duration is required');
      }

      // Step 5: Save game to database (with or without Stripe IDs)
      const insertData = {
        ...cleanedGameData,
      ...
      console.log('📝 Inserting game data:', {
        ...
        has_schedule: !!insertData.schedule
      });
      ...
      console.log('✅ Game inserted successfully:', data.id);

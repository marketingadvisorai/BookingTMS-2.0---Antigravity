...
export function KPICard({
  ...
  const resolvedTrend = trend ?? (change !== undefined ? { value: change, isPositive: change >= 0 } : undefined);
  const isPositive = resolvedTrend ? resolvedTrend.isPositive ?? resolvedTrend.value >= 0 : undefined;

  const displayValue = typeof value === 'number' ? value.toLocaleString() : value;
  const formattedValue = `${valuePrefix}${displayValue}${valueSuffix}`;
      ...
      <CardContent className="relative p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-2xl sm:text-3xl text-gray-900 dark:text-white mb-3 truncate">{formattedValue}</p>
            {resolvedTrend && (
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <span className={`text-sm ${isPositive ? 'text-emerald-600 dark:text-emerald-500' : 'text-red-600 dark:text-red-500'}`}>
                  {Math.abs(resolvedTrend.value)}%
                </span>
                <span className="text-xs sm:text-sm text-gray-500 dark:text-[#737373]">
                  {resolvedTrend.label ?? period ?? 'vs last month'}
                </span>
              </div>
            )}
          </div>

...
export default function AddServiceItemWizard({ onComplete, onCancel, initialData, mode = 'create', theme, embedContext, venueType }: AddServiceItemWizardProps) {
  ...
  const validateGameData = () => {
    ...
    if (!gameData.name || gameData.name.trim() === '') errors.push(`${t.singular} name is required`);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 6:
        ...
        />;
      case 7:
        return <Step7WidgetEmbed gameData={gameData} updateGameData={updateGameData} theme={theme} embedContext={embedContext} t={t} />;
      case 8:
        return <Step8Review gameData={gameData} t={t} />;
      default:
        return null;
    }
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Set the minimum and maximum number of players for this {t.singular.toLowerCase()}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-900">
              💡 <strong>Capacity Summary:</strong> This {t.singular.toLowerCase()} can accommodate{' '}
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configure {t.singular.toLowerCase()} duration, difficulty, and requirements</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This is the main image that will be displayed on your booking page
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-6">
            <div className="relative w-40 h-40 bg-gray-100 rounded-lg overflow-hidden border-2 border-dashed border-gray-300 flex items-center justify-center group">
              {gameData.coverImage ? (
                <>
                  <img
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add more photos to showcase your {t.singular.toLowerCase()}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <Label htmlFor="gallery-upload" className="cursor-pointer">
              <div className="aspect-square bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center hover:bg-gray-100 transition-colors">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add a YouTube or Vimeo link to a trailer or walkthrough
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2">
            <Input
              ...
              value={gameData.videos[0] || ''}
              onChange={(e) => {
                const newVideos = [...gameData.videos];
                newVideos[0] = e.target.value;
                updateGameData('videos', newVideos.filter(Boolean));
              }}
            />
          </div>
        </CardContent>
...
function Step5Schedule({ gameData, updateGameData, t }: StepProps) {
  ...
  const updateCustomHours = (day: string, field: 'startTime' | 'endTime', value: string) => {
    const updatedCustomHours = {
      ...
      [day]: {
        ...gameData.customHours[day],
        [field]: value
      }
    };
    updateGameData('customHours', updatedCustomHours);
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select the days when this {t.singular.toLowerCase()} is available</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-amber-200 bg-amber-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-amber-900">
            ...
            Widget Configuration Skipped
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-amber-800">
            Widget configuration is currently disabled as we focus on the core booking functionality.
            You can proceed to review and save your {t.singular.toLowerCase()}.
          </p>
        </CardContent>
      </Card>
    </div>
...
function Step8Review({ gameData, t }: { gameData: GameData; t: any }) {
  ...
  const validateGameData = () => {
    ...
    if (!gameData.name || gameData.name.trim() === '') {
      errors.push(`${t.singular} name is required`);
    }
    ...
    if (gameData.galleryImages.length === 0) {
      warnings.push(`No gallery images - consider adding photos to showcase your ${t.singular.toLowerCase()}`);
    }
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Review Your {t.singular}</CardTitle>
          <CardDescription>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <span className={`text-sm ${gameData.name ? 'text-green-700 line-through' : 'text-blue-700'}`}>
                        Set {t.singular.toLowerCase()} name and description
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className={`text-sm ${gameData.duration > 0 ? 'text-green-700 line-through' : 'text-blue-700'}`}>
                        Set {t.singular.toLowerCase()} duration in Step 3
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700">
                    Your {t.singular.toLowerCase()} will be immediately available for booking once published.
                  </p>
                </div>
              </div>
            </CardContent>

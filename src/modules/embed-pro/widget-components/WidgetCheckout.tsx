  ...
  return (
    <form onSubmit={handleSubmit} className="p-3 sm:p-4 md:p-5">
      ...
      <div className="flex items-center gap-2 mb-4 md:mb-5">
        ...
        <div>
          <h3 className="font-semibold text-gray-800 text-sm sm:text-base md:text-lg">Your Information</h3>
          <p className="text-[10px] sm:text-xs text-gray-500">We'll send your confirmation here</p>
        </div>
      </div>
      ...
      <div className="space-y-3 md:space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="relative group">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-gray-600 transition-colors" />
              <input
                type="text"
                value={formData.firstName}
                onChange={(e) => handleChange('firstName', e.target.value)}
                className={`w-full pl-10 pr-3 py-2.5 md:py-3 border rounded-xl text-sm md:text-base
                  ${errors.firstName ? 'border-red-500 bg-red-50/50' : 'border-gray-200 bg-white/80'}
            ...
            <div className="relative group">
              <input
                ...
                className={`w-full px-3 py-2.5 md:py-3 border rounded-xl text-sm md:text-base
                  ${errors.lastName ? 'border-red-500 bg-red-50/50' : 'border-gray-200 bg-white/80'}
                  focus:outline-none focus:ring-2 focus:ring-offset-1 focus:border-transparent
                  hover:border-gray-300 transition-all duration-150`}
                style={{ ['--tw-ring-color' as any]: style.primaryColor }}
                placeholder="Doe"
        ...
        <div>
          ...
          <div className="relative group">
            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-gray-600 transition-colors" />
            <input
              type="email"
              value={formData.email}
              onChange={(e) => handleChange('email', e.target.value)}
              className={`w-full pl-10 pr-3 py-2.5 md:py-3 border rounded-xl text-sm md:text-base
                ${errors.email ? 'border-red-500 bg-red-50/50' : 'border-gray-200 bg-white/80'}
                focus:outline-none focus:ring-2 focus:ring-offset-1 focus:border-transparent
                hover:border-gray-300 transition-all duration-150`}
              style={{ ['--tw-ring-color' as any]: style.primaryColor }}
        ...
        <div>
          ...
          <div className="relative group">
            <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-gray-600 transition-colors" />
            <input
              type="tel"
        ...
        <div>
          ...
          <div className="relative group">
            <MessageSquare className="absolute left-3 top-3 w-4 h-4 text-gray-400 group-focus-within:text-gray-600 transition-colors" />
            <textarea
              value={formData.notes}
              onChange={(e) => handleChange('notes', e.target.value)}
              rows={2}
              className="w-full pl-10 pr-3 py-2.5 md:py-3 border border-gray-200 bg-white/80 rounded-xl text-sm md:text-base
                focus:outline-none focus:ring-2 focus:ring-offset-1 focus:border-transparent
                hover:border-gray-300 transition-all duration-150 resize-none"
              style={{ ['--tw-ring-color' as any]: style.primaryColor }}
      ...
      {/* Security Badge */}
      <div className="flex items-center justify-center gap-1.5 mt-4 text-[10px] sm:text-xs text-gray-400">
      ...
      <div className="flex gap-3 mt-4 md:mt-5">
        <button
          type="button"
          onClick={onBack}
          className="flex-1 min-h-[44px] md:min-h-[48px] py-3 px-4 border border-gray-200 rounded-xl font-medium
                     text-gray-700 hover:bg-gray-50 active:scale-[0.98] transition-all duration-150 touch-manipulation

...
export default function Step6PaymentSettings({
  ...
  /**
   * Create new Stripe product with multi-tier pricing (adult, child, custom)
   * Uses lookup keys for real-time price updates
   */
  const handleCreateStripeProduct = async () => {
    ...
    console.log('🚀 Starting Stripe product creation with multi-tier pricing...');
    console.log('📊 Activity Data:', {
      name: activityData.name,
      adultPrice: activityData.adultPrice,
      childPrice: activityData.childPrice,
      customFields: activityData.customCapacityFields,
    });
    ...
    try {
      toast.loading('Creating Stripe product with pricing tiers...', { id: 'stripe-create' });

      // Build custom capacity fields for pricing
      ...
      // Step 2: Create multi-tier prices with lookup keys (if activity ID exists)
      ...
      if (activityData.id) {
        try {
          ...
          console.log('✅ Multi-tier prices created:', multiTierPrices);
        } catch (tierError) {
          console.warn('⚠️ Multi-tier price creation failed, using single price:', tierError);
        }
      }

      // Update activity data with Stripe IDs and pricing structure
      ...
      const tierCount = (multiTierPrices?.adult ? 1 : 0) + 
                        (multiTierPrices?.child ? 1 : 0) + 
                        (multiTierPrices?.custom?.length || 0);
      
      toast.success(`Stripe product created with ${tierCount} pricing tier(s)!`, { id: 'stripe-create' });
    } catch (error: any) {
  ...
  const handleLinkExistingProduct = async () => {
    ...
    try {
      ...
      // Convert array prices to structured format
      const convertedPrices: ActivityData['stripePrices'] = {
        adult: null,
        child: null,
        custom: [],
      };

      // Map prices by their metadata type
      for (const price of result.prices) {
        ...
        if (tierType === 'adult' || (!convertedPrices.adult && tierType !== 'child')) {
          convertedPrices.adult = priceData;
        } else if (tierType === 'child') {
          convertedPrices.child = priceData;
        } else if (tierType === 'custom') {
          convertedPrices.custom?.push({
            ...priceData,
      ...
      // Update activity data with product and structured prices
      const updatedData: ActivityData = {
        ...activityData,
        stripeProductId: result.productId,
        stripePrices: convertedPrices,
        stripePriceId: result.priceId || convertedPrices.adult?.price_id || result.prices[0]?.priceId,
        stripeCheckoutUrl: stripeCheckoutUrl.trim() || undefined,
        stripeSyncStatus: 'synced' as SyncStatus,
        stripeLastSync: new Date().toISOString(),
      };
  ...
  const handleRefreshSync = async () => {
    ...
    try {
      ...
      for (const p of prices) {
        const tierType = p.metadata?.tier_type || 'adult';
        const priceData = {
          price_id: p.id,
        ...
        if (tierType === 'adult' || (!convertedPrices.adult && tierType !== 'child')) {
          ...
          convertedPrices.custom?.push({
            ...priceData,
            id: p.metadata?.custom_id || '',
            name: p.metadata?.display_name || 'Custom',
            min: parseInt(p.metadata?.min_quantity || '0'),
            max: parseInt(p.metadata?.max_quantity || '10'),
          });
        }
      }
      ...
      const priceCount = (convertedPrices.adult ? 1 : 0) + 
                         (convertedPrices.child ? 1 : 0) + 
                         (convertedPrices.custom?.length || 0);
      toast.success(`Synced! Found ${priceCount} price tier(s)`, { id: 'stripe-sync' });
    } catch (error: any) {
